...
export const CropAvatar = ({ uploadImage }) => {
    ...
    const [crop, setCrop] = useState({ unit: '%', x: 10, y: 10, width: 80, height: 80, aspect: 4 / 4 });
    ...
    const onImageLoaded = useCallback((img) => { cropRef.current = img }, []);
        ...
        <>
            {(src &&
                ...
                <label className={styles.addImgButtonLabel} htmlFor="contained-button-file">
                    <Button className={styles.addImgButton} variant="containedPurple" onClick={handleClick}>Выбрать файл</Button>
